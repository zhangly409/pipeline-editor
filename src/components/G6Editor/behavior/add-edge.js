    ...
    if (item && item.getType() === 'node') {
      ...
      if (activeItem) {
        ...
        if (this.edge) {
          ...
          let startModel = startItem.getModel()
          let endModel = item.getModel()
          const model = {
            id: 'edge' + uniqueId(),
            source: startItem,
            target: item,
            sourceId: startModel.id,
            targetId: endModel.id,
            start: startPoint,
            end: endPoint,
            type: 'edge',
          }
          if (startModel.id == endModel.id) {
            ...
            if (this.graph.cfg.container.className == 'graph-container') {
              eventBus.$emit('addItem', model)
            } else if (this.graph.cfg.container.className == 'task-graph-container') {
              eventBus.$emit('addTaskItem', model)
            }
          }
        }
      } else {
  ...
  onMouseover(e) {
    ...
    if (item && item.getType() === 'node') {
      if (e.target.attrs.isInPointOut && !this.hasTran) {
        ...
        if (!matrix) matrix = G6.Util.mat3.create()
