...
class command {
    ...
    executeCommand(key, datas) {
        ...
        this.editor.emit('task' + key, { undoList: this.undoList, redoList: this.redoList })
    }
    doCommand(key, data) {
        switch (key) {
            case 'add':
            ...
            case "update":
                ...
                break
            case "delete":
                eventBus.$emit('tasknodeselectchange', { target: data.item, select: false })
                this.remove(data)
                break
        }
    }
    ...
    undo() {
        ...
        this.editor.emit('task' + undoData.key, { undoList: this.undoList, redoList: this.redoList })
    }
    doundo(key, data) {
        switch (key) {
            case 'add':
    ...
    redo() {
        ...
        this.editor.emit('task' + redoData.key, { undoList: this.undoList, redoList: this.redoList })
    }
    doredo(key, data) {
        switch (key) {
            case 'add':
